...
3. **Macro editing** - v0.4 in progress: add one narrow
   `EditMacroProcedure` operation for an isolated `.xlsm` saved only as a
   `Copy`. It returns a bounded requested-procedure source/hash in Plan; Apply
   requires the expected hash and a complete replacement, returns no source,
   and may optionally run the no-argument procedure. Trust access remains
   user-controlled. Gate: a disposable `.xlsm` edit, run, save, and reopen
   test fails closed on dialogs and leaves no owned Excel process.
